// Chooses the next Broiler.Plate preview version for the Publish workflow.
//
// The workflow does not push its packages anywhere, so the record of what has been built is
// the repository's tags: every successful run tags its commit plate-v<version>. The next
// version is the configured floor, BroilerPlateVersion from Directory.Build.props, raised
...
export const tagPrefix = 'plate-v';
...
function readConfiguredVersion() {
  const output = execFileSync('dotnet', [
    'msbuild', 'src/Broiler.Plate/Broiler.Plate.Core.csproj', '-nologo',
    '-getProperty:BroilerPlateVersion',
